...
datacsv = pd.read_csv(str(pathlib.Path.cwd())+'\\night_market_edited_c2.csv')
datacsv = datacsv.fillna(0)
labelencoder = LabelEncoder()

datacsv['夜市名稱'] = labelencoder.fit_transform(datacsv['夜市名稱'])
datacsv['調味名稱'] = labelencoder.fit_transform(datacsv['調味名稱'])
datacsv['攤商名稱'] = labelencoder.fit_transform(datacsv['攤商名稱'])
datacsv['品牌'] = labelencoder.fit_transform(datacsv['品牌'])
datacsv['來源(產地)'] = labelencoder.fit_transform(datacsv['來源(產地)'])
...
with open('night_market_dec_tree_dataset_c2.json', 'w', encoding='utf-8') as f:
